...
export default function TaskModal({ isOpen, onClose, projectId, projectMembers, onTaskCreated }: TaskModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Full API URL:', `${apiClient.defaults.baseURL}/projects/${projectId}/tasks`);
    ...
    try {
      ...
      if (err.response?.status === 401) {
        ...
        errorMessage = err.response?.data?.errors?.join(', ') || 'Validation failed';
              ...
              <div className="mb-4 bg-red-50 border-l-4 border-red-500 text-red-700 px-4 py-3 rounded">
                <div className="flex">
                  ...
                  <div>
                    ...
                    <p className="text-xs mt-1">Check browser console (F12) for detailed error information.</p>
                  </div>
                </div>
              </div>
            )}
